...
describe("Claims", function () {
  ...
  it("ongoing grant claim should succeed", async function () {
    ...
    await expect(project.connect(user1).claimVestedTokens(0))
      .to.emit(project, "GrantClaimed")
      .withArgs(0, user1.address, amount.div(2));
  ...
  it("invalid pool index should fail", async function () {
    ...
    await expect(project.connect(user1).claimVestedTokens(1)).to.rejectedWith(
  ...
  it("invalid grantee should fail", async function () {
    ...
    await expect(project.connect(user2).claimVestedTokens(0)).to.rejectedWith(
      "VestingPeriod::claimVestedTokens: amountVested is 0'"
    );
  });

  it("claimable balance 0 should fail", async function () {
    await expect(project.connect(user1).claimVestedTokens(0)).to.rejectedWith(
      "VestingPeriod::claimVestedTokens: amountVested is 0'"
    );
  });
});

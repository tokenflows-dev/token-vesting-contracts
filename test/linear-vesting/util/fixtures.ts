...
async function deployProjectFactoryWithProject() {
  const { projectFactory, projectBase, owner, erc20, manager, user1, user2 } =
    await deployProjectFactory();
...
async function deployProjectFactoryWithProjectAndPool() {
  const {
    ...
    user1,
    user2,
  } = await deployProjectFactoryWithProject();
...
export {
  deployProjectFactory,
  deployProjectFactoryWithProject,
  deployProjectFactoryWithProjectAndPool,
};
